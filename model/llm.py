...
def tokenize_texts(X, model_name, max_seq_length):
    try:
        tokenizer = AutoTokenizer.from_pretrained(model_name, use_auth_token="XXX")
...
def initialize_model(model_name, num_labels=3):
    try:
        model = AutoModelForSequenceClassification.from_pretrained(
            model_name, num_labels=num_labels, use_auth_token="XXX"
        )
        model.to(device)
        return model
    except Exception as e:
...
def train_model(model, train_loader, optimizer, scheduler, epochs):
    try:
        ...
        for epoch in range(epochs):
            ...
            logging.info(f"Epoch {epoch + 1}/{epochs} completed with SERENGETI LLM average loss: {avg_loss}")
        total = time.time() - start
        logging.info("The total amount of time it took to train the SERENGETI LLM is {}".format(total))
        return True
    except Exception as e:
...
def main():
    try:
        ...
        input_ids, attention_mask, _ = tokenize_texts(X, model_name, max_seq_length)
